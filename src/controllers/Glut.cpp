		...
		unsigned char key, int x, int y)
{
	...
	if (key_i == 0)
	{
		...
		{
			vector<Reconstructor::Person*> persons = scene3d.getReconstructor().getPersons();
			for (int p = 0; p < persons.size(); p++)
			{
				for (int i = 0; i < persons[p]->path.size(); i++)
				{
					persons[p]->path[i].x = INFINITY;
					persons[p]->path[i].y = INFINITY;
					persons[p]->path_computed[i] = false;
				}
			}
		}
		else if (key == 'n' || key == 'N')
		...
		int v)
{
	...
	else if (scene3d.getHThreshold() != scene3d.getPHThreshold()
		|| scene3d.getSThreshold() != scene3d.getPSThreshold()
		|| scene3d.getVThreshold() != scene3d.getPVThreshold()
		|| scene3d.getCustomThreshold() != scene3d.getPCustomThreshold()
		)
	{
		...
		scene3d.setPCustomThreshold(scene3d.getCustomThreshold());
	...
	if (!canvas.empty())
	{
		Mat fff = canvas.clone();
		bilateralFilter(canvas, fff, BLURDIAMETER, BLURSIGMACOLOR, BLURSIGMASPACE);
		canvas = fff;
	}
...
void Glut::drawVoxels()
{
	...
	vector<Reconstructor::Cluster*> voxel_clusters = m_Glut->getScene3d().getReconstructor().getClusters();
	vector<Reconstructor::Person*> persons = m_Glut->getScene3d().getReconstructor().getPersons();
	
#if VOXELCLUSTERING
	// draw clusters
	for (int i = 0; i < voxel_clusters.size(); i++)
	{
		glColor4f(
			voxel_clusters[i]->person->color.val[2],
			voxel_clusters[i]->person->color.val[1],
			voxel_clusters[i]->person->color.val[0],
			0.5f
		);
		for (int v = 0; v < voxel_clusters[i]->data.size(); v++)
		{
			Reconstructor::Voxel* voxel = voxel_clusters[i]->data[v];
	...
	{
		glColor4f(
			persons[i]->color.val[2],
			persons[i]->color.val[1],
			persons[i]->color.val[0],
			0.5f
		);
		for (int p = 1; p < persons[i]->path.size(); p++)
		{
			//draw vertex position
			if (persons[i]->path_computed[p - 1] && persons[i]->path_computed[p])
			{
				glVertex3f(persons[i]->path[p - 1].x, persons[i]->path[p - 1].y, 0.0f);
				glVertex3f(persons[i]->path[p].x, persons[i]->path[p].y, 0.0f);
			}
		}
	}
	glEnd();
	...
	if (
		m_Glut->getScene3d().getCurrentFrame() == m_Glut->getScene3d().getNumberOfFrames() - 5 ||
		m_Glut->getScene3d().getCurrentFrame() == 20 ||
		m_Glut->getScene3d().getCurrentFrame() == 200
		)
	{
		m_Glut->savePaths();
	}
...
#if VOXELCOLORING
		glColor4f(0.5f, 0.5f, 0.5f, 0.5f);
		for (int c = 0; c < m_Glut->getScene3d().getCameras().size(); c++)
		{
			if (voxels[v]->closest_to_camera[c])
			{
...
// for each camera it computes a set of histograms for a set of hard-coded frames for each camera
void Glut::initHistograms()
{
	const int frames[] = { 199, 1, 531, 506 };
	vector<Reconstructor::Cluster*> clusters;
	vector<vector<Mat>> histograms;
	Camera* cam;
	int total_samples_ignored = 0;

	// histogram parameters
	for (int c = 0; c < ( int) m_Glut->getScene3d().getCameras().size(); c++)
	{
		printf("Camera %d: Computing histograms...\n", c + 1);
		histograms.push_back(vector<Mat>());
		cam = m_Glut->getScene3d().getCameras()[c];
		m_scene3d.setCurrentFrame(frames[c]);
		m_scene3d.processFrame();
		m_scene3d.getReconstructor().update();
		m_scene3d.getReconstructor().m_initialize_step++;
		clusters = m_scene3d.getReconstructor().getClusters();
		// for each cluster compute the histogram
		for (int i = 0; i < clusters.size(); i++)
		{
			histograms[c].push_back(clusters[i]->histogram);
		}
	}
	// process camera histograms
	printf("---=== Done! ===---\n\n");

	std::vector<Reconstructor::Person*> people = m_Glut->getScene3d().getReconstructor().getPersons();

	people[0]->histograms.push_back(histograms[INITCAMERA][0]);
	people[1]->histograms.push_back(histograms[INITCAMERA][1]);
	people[2]->histograms.push_back(histograms[INITCAMERA][2]);
	people[3]->histograms.push_back(histograms[INITCAMERA][3]);

	for (int c = 0; c < histograms.size(); c++)
	{
		if (c == INITCAMERA) continue;
		for (int i = 0; i < histograms[c].size(); i++)
		{
			Mat h = histograms[c][i];
			double p_min_dst = INFINITY;
			int p_idx = -1;
			for (int p = 0; p < 4; p++)
			{
				double dst = compareHist(people[p]->histograms[0], h, CV_COMP_CHISQR); // 4 8 2 2
				if (dst < p_min_dst)
				{
					p_min_dst = dst;
					p_idx = p;
			...
			people[p_idx]->histograms.push_back(h);
		}
	}

	for (int p = 0; p < people.size(); p++)
	{
		printf("%d ",(int) people[p]->histograms.size());
	}

	int min_hs = MAXINT;
	for (int p = 0; p < people.size(); p++)
	{
		min_hs = min(min_hs, ( int) people[p]->histograms.size());
	}
	for (int p = 0; p < people.size(); p++)
	{
		if (people[p]->histograms.size() > min_hs)
		{
			people[p]->histograms.erase(
				people[p]->histograms.begin() + min_hs,
				people[p]->histograms.end()
			);
		}
	}
	m_scene3d.getReconstructor().m_initialized = true;
	//m_scene3d.setPaused(false);
	m_scene3d.setCurrentFrame(1);
	m_scene3d.processFrame();
	m_scene3d.getReconstructor().update();
...
{
	vector<Reconstructor::Person*> persons = m_Glut->getScene3d().getReconstructor().getPersons();
	Mat output_image = Mat::zeros(PATHIMAGEHEIGHT, PATHIMAGEWIDTH, CV_8UC3);
	for (int p = 0; p < persons.size(); p++)
	{
		vector<Point2d> valid_path_points;
		Reconstructor::Person* person = persons[p];
		// get valid path positions
		for (int i = 0; i < person->path.size(); i++)
		{
			if (person->path_computed[i])
			{
				valid_path_points.push_back(person->path[i]);
			}
		}
		// for each valid path position create a position array to normalize
		Mat path_x = Mat(valid_path_points.size(), 1, CV_64F);
		Mat path_y = Mat(valid_path_points.size(), 1, CV_64F);
		for (int i = 0; i < valid_path_points.size(); i++)
		{
			path_x.at<double>(i) = valid_path_points[i].x;
			path_y.at<double>(i) = valid_path_points[i].y;
		}
		//normalize(path_x, path_x, 0, PATHIMAGEWIDTH, NORM_MINMAX, -1, Mat());
		//normalize(path_y, path_y, 0, PATHIMAGEHEIGHT, NORM_MINMAX, -1, Mat());
		path_x /= 3;
		path_y /= 4;
		path_x += 1250;
		path_y += 750;
		// plot each point to output image
		for (int yuuyyyy = 1; yuuyyyy < valid_path_points.size(); yuuyyyy++)
		{
			Point2d xy0 = Point2d(path_y.at<double>(yuuyyyy-1), path_x.at<double>(yuuyyyy-1));
			Point2d xy1 = Point2d(path_y.at<double>(yuuyyyy), path_x.at<double>(yuuyyyy));
			Vec3b color = Vec3b(
				(uchar)(person->color.val[0] * 255),
				(uchar)(person->color.val[1] * 255),
				(uchar)(person->color.val[2] * 255)
			);
			line(output_image, xy0, xy1, color);
			//output_image.at<Vec3b>(xy) = color;
		}
	}
	imwrite("C:/Users/tniko/Desktop/Uni/Computer Vision/assignment_3/VoxelReconstruction/data/paths.png", output_image);
}
